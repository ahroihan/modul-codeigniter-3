...
<div class="content-wrapper">
	<section class="content-header">
		...
		<ol class="breadcrumb">
			...
			<li><a href="<?php echo site_url($uriKu . 'pengembalian'); ?>"> Permintaan</a></li>
		...
		<div class="box">
				...
				<div class="col-md-12">
					<div class="row">
						<div class="col-md-6 col-centered">
							...
							<div class="form-group">
								<label for="pengguna" class="col-sm-3 control-label">ID Permintaan</label>
								<div class="col-sm-9">
									<?php
									if ($segment4) {
										$data = array(
											site_url($uriKu . 'permintaan/edit/' . $pengembalian['id']) => 'PILIH'
										);
										$data = array(
											site_url($uriKu . 'permintaan/edit/' . $pengembalian['id'] . '/' . $pengembalian['peminjaman']) =>  $pengembalian['peminjaman']
										);
										foreach ($peminjaman as $row) {
											$data[site_url($uriKu . 'permintaan/edit/' . $pengembalian['id'] . '/' . $row['id'])] = $row['id'];
										}
										echo form_dropdown(
											'peminjaman',
											$data,
											site_url($uriKu . 'permintaan/edit/' . $pengembalian['id'] . '/' . $segment4),
											'onChange="window.location.replace(this.options[this.selectedIndex].value)" class="autocomplete form-control required"'
										);
									} else {
										$data = array(site_url($uriKu . 'permintaan/add') => 'PILIH');
										foreach ($peminjaman as $row) {
											$data[site_url($uriKu . 'permintaan/add/' . $row['id'])] = $row['id'];
										}
										echo form_dropdown(
											'peminjaman',
											$data,
											site_url($uriKu . 'permintaan/add/' . $segment3),
											'onChange="window.location.replace(this.options[this.selectedIndex].value)" class="autocomplete form-control required"'
										);
									}
									?>
			...
			<div class="box-footer">
				<div class="col-md-offset-5">
					<a href="<?php echo site_url($uriKu . 'pengembalian'); ?>" class="btn btn-flat btn-default" data-toggle="tooltip" data-placement="top" title="Kembali">
